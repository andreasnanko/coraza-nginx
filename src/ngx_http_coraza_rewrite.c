...
ngx_http_coraza_rewrite_handler(ngx_http_request_t *r)
{
    ...
    /* Check if $waf_zone_id is "BYPASS" (set by set_by_lua_block when
     * the client IP is in the global allowlist). Skip all WAF processing. */
    {
        u_char lowcase[11];
        ngx_str_t var_name;
        ngx_uint_t key;
        ...
        if (vv && !vv->not_found && vv->len == 6
            && ngx_strncmp(vv->data, "BYPASS", 6) == 0) {
            return NGX_DECLINED;
        }
    }
